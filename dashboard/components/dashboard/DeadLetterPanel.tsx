    ...
    <div className="rounded-xl2 border border-critical/30 bg-critical/5 p-4">
      ...
      <div className="space-y-2 max-h-72 overflow-y-auto">
        {tasks.map((t) => (
          <div
            key={t.task_id}
            className="rounded-lg bg-panel border border-grid px-3 py-2.5 text-xs"
          >
            <div className="flex items-center justify-between gap-2 mb-1">
              <span className="mono text-haze uppercase">{t.mode}</span>
              {t.requested_at && (
            ...
            <button
              ...
              className="text-haze hover:text-mist transition disabled:opacity-50"
            >
